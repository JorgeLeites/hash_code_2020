...
while remaining_time > 0:
    ...
    for library in libraries:
        if library.signup_duration < remaining_time:
            ...
            t = (remaining_time - library.signup_duration)

            books_library_can_process = min(
                t * library.books_per_day, library.number_of_books
            )

            book_scores = (books[index] for index in library.books[0:books_library_can_process])
            score = sum(book_scores)

            new_libraries = [library for library in libraries if library.signup_duration <= remaining_time]
            for new_library in new_libraries:
                new_library_books = [book for book in library.books if book not in books_seen]
                new_library_number_of_books = len(library.books)
                books_new_library_can_process = min(
                    t * new_library.books_per_day, new_library_number_of_books
                )
                new_book_scores = (books[index] for index in new_library_books[0:books_new_library_can_process])
                score += sum(new_book_scores)
